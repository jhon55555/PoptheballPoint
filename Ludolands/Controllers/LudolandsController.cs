using Ludolands.Helpers;
using Microsoft.AspNetCore.Mvc;
using Models.Ludolands;
using Newtonsoft.Json;
using Services.Common;
using Services.Jwt;
using Services.Ludolands;
using System;
using System.Data;
using System.IO;
...
namespace Ludolands.Controllers
{

    [ApiController]
    [Route("api/game")]
    public class LudolandsController : ControllerBase
    {//test
        private readonly ITokenManager _tokenManager;
        private readonly IJwtHandler _jwtHandler;
        private readonly ILudolandsService _popballservice;
        private readonly ErrorLog _Elog;
        private readonly HttpHelper _httpHelper;
        private readonly Config _config;
        //private readonly IHostingEnvironment _env;ajksd
        public LudolandsController(ITokenManager tokenManager, IJwtHandler jwtHandler, /*IHostingEnvironment env,*/ ILudolandsService popballservice, ErrorLog Elog, Config config, HttpHelper httpHelper)
        {
        ...
        public async Task<IActionResult> BetResult([FromBody] BalanceCallback balanceCallback)
        {
            ...
            try
            {
                ...
                if (TblData != null && TblData.Tables.Count > 0 && TblData.Tables[TblData.Tables.Count - 1].Columns.Contains("id") && TblData.Tables[TblData.Tables.Count - 1].Rows[0]["id"].ToString() == "-1")
                {
                    _Elog.WriteLogAll("BetResultda" + TblData.Tables[TblData.Tables.Count - 1].Rows[0]["MSG"].ToString(), JsonConvert.SerializeObject(obj));
                    //return Return400("Data Error");
                    var json = new
                    {
                        ErrorCode = 1,
                    ...
                    _Elog.WriteLogAll("BetResult21 : ", JsonConvert.SerializeObject(json));
                    var balde = EncryptString(JsonConvert.SerializeObject(json), _config.Secret);
                    var res1 = DecryptString(balde, _config.Secret);
                    _Elog.WriteLogAll("BetResult213 : ", JsonConvert.SerializeObject(res1));
                    return Ok(new { Body = balde });
                }
                if (TblData == null || TblData.Tables.Count <= 0)
                {
                    var json = new
                    {
                        ErrorCode = 1,
                    ...
                    _Elog.WriteLogAll("BetResult22 : ", JsonConvert.SerializeObject(json));
                    var balde = EncryptString(JsonConvert.SerializeObject(json), _config.Secret);
                    var res1 = DecryptString(balde, _config.Secret);
                    _Elog.WriteLogAll("BetResult214 : ", JsonConvert.SerializeObject(res1));
                    return Ok(new { Body = balde });
                }
                //return Return300("No Data Found.");
                if (TblData.Tables[0].Rows.Count <= 0)
                {
                    var json = new
                    {
                        ErrorCode = 1,
                    ...
                    _Elog.WriteLogAll("BetResult23 : ", JsonConvert.SerializeObject(json));
                    var balde = EncryptString(JsonConvert.SerializeObject(json), _config.Secret);
                    var res1 = DecryptString(balde, _config.Secret);
                    _Elog.WriteLogAll("BetResult215 : ", JsonConvert.SerializeObject(res1));
                    return Ok(new { Body = balde });
                }
                //return Return300("No Record Found.");
                if (TblData != null && TblData.Tables.Count > 0 && TblData.Tables[0].Rows.Count > 0 && TblData.Tables[0].Rows[0]["id"].ToString() == "0" && TblData.Tables[0].Rows[0]["istransfer"].ToString() == "1")
                {
                    ...
                    if (TblData2 != null && TblData2.Tables.Count > 0 && TblData2.Tables[TblData2.Tables.Count - 1].Columns.Contains("id") && TblData2.Tables[TblData2.Tables.Count - 1].Rows[0]["id"].ToString() == "-1")
                    {
                        _Elog.WriteLogAll("BetUrlda" + TblData2.Tables[TblData2.Tables.Count - 1].Rows[0]["MSG"].ToString(), JsonConvert.SerializeObject(obj));
                        //return Return400("Data Error");
                        var json = new
                        {
                            ErrorCode = 1,
                    ...
                    if (TblData2 == null || TblData2.Tables.Count <= 0)
                    {
                        var json = new
                        {
                            ErrorCode = 1,
                    ...
                    //return Return300("No Data Found.");
                    if (TblData2.Tables[0].Rows.Count <= 0)
                    {
                        var json = new
                        {
                            ErrorCode = 1,
                ...
                else
                {
                    ...
                    if (TblData1 != null && TblData1.Tables.Count > 0 && TblData1.Tables[TblData1.Tables.Count - 1].Columns.Contains("id") && TblData1.Tables[TblData1.Tables.Count - 1].Rows[0]["id"].ToString() == "-1")
                    {
                        ...
                        _Elog.WriteLogAll("BetUrl21 : ", JsonConvert.SerializeObject(jsonq));
                        var balde = EncryptString(JsonConvert.SerializeObject(jsonq), _config.Secret);
                        var res1 = DecryptString(balde, _config.Secret);
                        _Elog.WriteLogAll("BetUrl213 : ", JsonConvert.SerializeObject(res1));
                        return Ok(new { Body = balde });
                    }
                    if (TblData1 == null || TblData1.Tables.Count <= 0)
                    {
                        ...
                        _Elog.WriteLogAll("BetUrl22 : ", JsonConvert.SerializeObject(jsona));
                        var balde = EncryptString(JsonConvert.SerializeObject(jsona), _config.Secret);
                        var res1 = DecryptString(balde, _config.Secret);
                        _Elog.WriteLogAll("BetUrl214 : ", JsonConvert.SerializeObject(res1));
                        return Ok(new { Body = balde });
                    }
                    ...
                    if (TblData1.Tables[0].Rows.Count <= 0)
                    {
                        ...
                        _Elog.WriteLogAll("BetUrl23 : ", JsonConvert.SerializeObject(jsonc));
                        var balde = EncryptString(JsonConvert.SerializeObject(jsonc), _config.Secret);
                        var res1 = DecryptString(balde, _config.Secret);
                        _Elog.WriteLogAll("BetUrl215 : ", JsonConvert.SerializeObject(res1));
                        return Ok(new { Body = balde });
                    }
                    ...
                    {
                        ErrorCode = TblData != null && TblData.Tables.Count > 0 && TblData.Tables[0].Rows.Count > 0 && TblData.Tables[0].Columns.Contains("id") ? Convert.ToInt32(TblData.Tables[0].Rows[0]["id"]) : 1,
                        Balance = TblData1 != null && TblData1.Tables.Count > 0 && TblData1.Tables[0].Rows.Count > 0 && TblData1.Tables[0].Columns.Contains("general") ? (!string.IsNullOrEmpty(TblData1.Tables[0].Rows[0]["general"].ToString()) ? Convert.ToDecimal(TblData1.Tables[0].Rows[0]["general"]) : 0) - (!string.IsNullOrEmpty(TblData1.Tables[0].Rows[0]["exposer"].ToString()) ? Convert.ToDecimal(TblData1.Tables[0].Rows[0]["exposer"]) : 0) : 0,
                        Currency = TblData1.Tables[0].Rows[0]["Currency"].ToString()
                    };
                    _Elog.WriteLogAll("BetResult : ", JsonConvert.SerializeObject(json));
                    var balde1 = EncryptString(JsonConvert.SerializeObject(json), _config.Secret);
