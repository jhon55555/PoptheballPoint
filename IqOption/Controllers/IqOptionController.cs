using IqOption.Helpers;
using Microsoft.AspNetCore.Mvc;
using Models.IqOption;
using Newtonsoft.Json;
using Newtonsoft.Json.Linq;
using Services.Common;
using Services.IqOption;
using Services.Jwt;
using System;
using System.Data;
using System.IO;
...
namespace IqOption.Controllers
{
    ...
    {//test
        ...
        private readonly IIqOptionService _popballservice;
        private readonly ErrorLog _Elog;
        private readonly HttpHelper _httpHelper;
        private readonly Config _config;
        //private readonly IHostingEnvironment _env;ajksd
        public IqOptionController(ITokenManager tokenManager, IJwtHandler jwtHandler, /*IHostingEnvironment env,*/ IIqOptionService popballservice, ErrorLog Elog, Config config, HttpHelper httpHelper)
        {
        ...
        public async Task<IActionResult> BetUrl([FromBody] BalanceCallback balanceCallback)
        {
            ...
            try
            {
                ...
                var res = DecryptString(balanceCallback.Body, _config.Secret);
                _Elog.WriteLogAll("BetUrl+1", res);
                var obj = JsonConvert.DeserializeObject<IqBetResultres>(res);
                ...
                {
                    var TblData = await _popballservice.debit(obj.PlayerId, obj.Session, obj.GameId, obj.RoundId, obj.TransactionId, obj.Rate, obj.Stake, obj.Win, Convert.ToDouble(TblData1.Tables[0].Rows[0]["general"]) - Convert.ToDouble(TblData1.Tables[0].Rows[0]["exposer"]), obj.Created, obj.Updated, obj.market, obj.timespan, obj.Payout);
        ...
        public async Task<IActionResult> BetResult([FromBody] BalanceCallback balanceCallback)
        {
            ...
            try
            {
                ...
                //if (TblData1 != null && TblData1.Tables.Count > 0 && TblData1.Tables[0].Rows.Count > 0 && TblData1.Tables[0].Rows[0]["id"].ToString() == "1")
                //{
                var TblData = await _popballservice.credit(obj.PlayerId, obj.Session, obj.GameId, obj.RoundId, obj.TransactionId, obj.Reference, obj.ResultRate, obj.Stake, obj.Win, 0, obj.Created, obj.Updated, obj.Payout);
        ...
        public async Task<IActionResult> Rollback([FromBody] BalanceCallback balanceCallback)
        {
            ...
            try
            {
                ...
                //if (TblData1 != null && TblData1.Tables.Count > 0 && TblData1.Tables[0].Rows.Count > 0 && TblData1.Tables[0].Rows[0]["id"].ToString() == "1")
                //{
                var TblData = await _popballservice.Rollback(obj.PlayerId, obj.Session, obj.GameId, obj.RoundId, obj.TransactionId, obj.Reference, obj.ResultRate, obj.Stake, obj.Win, 0, obj.Created, obj.Updated, obj.Payout);
